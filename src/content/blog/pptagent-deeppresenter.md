...
date: 2026-08-15
summary: "系统讲解 PPTAgent 与 DeepPresenter 的生成流程、评测体系、Agent Harness 主循环和框架架构。"
...
1. 递增 `turn_count`，执行 `max_turns` 上限（剩余少于 2 轮时注入 "finish now" 提示）；
...
| `ToolAgents`（`tool_agents.py`） | Create/Reason | `image_generation` / `image_caption` / `document_summary` | 条件注册：配置了 t2i_model 才提供文生图；vision_model 提供图片分类+描述（Table/Chart/Logo，少于 50 词）；multiagent 模式提供长文档摘要 |
